...
select pg_temp.ok((select count(*) = 2 from public.transcript_segments where transcript_id = '50000000-0000-4000-8000-000000000003' and suggested_speaker_role = speaker_role and speaker_role in ('patient','clinician')) and (select speaker_identification_model = 'test-llm' and speaker_identified_at is not null and speakers_confirmed_at is not null from public.transcripts where id = '50000000-0000-4000-8000-000000000003'), 'LLM identification automatically assigns every transcript speaker');
select pg_temp.ok((select status = 'queued' and base_note_version = 0 from public.processing_jobs where transcript_id = '50000000-0000-4000-8000-000000000003' and kind = 'drafting'), 'Automatic speaker assignment queues an idempotent SOAP draft');
