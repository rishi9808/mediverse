...
5. Insert a `pending` transcript, insert segments, then mark it `ready`. Audio-backed transcripts require verified audio from that session. Seeded fictional transcripts use `source = 'seeded'` and no audio ID. Segment timestamps must fit the transcript. For the first generated note, one structured LLM request assigns Psychologist/Patient roles and creates the evidence-linked SOAP draft, removing a sequential provider round trip.
6. Validate the combined output before applying speaker roles or saving the draft. Standalone regeneration still sends the confirmed full transcript to the drafting provider. Save validated output using `save_note_revision`, with the last seen version (`0` for the first save). SQLSTATE `40001` means the screen is stale: reload before retrying. Every save increments the version. Referenced transcript text and roles freeze; corrections require a new transcript version and draft revision.
...
8. PDF rendering reads `approved_notes.snapshot`, including patient/psychologist identity, session metadata, approval time, and SOAP content. It excludes evidence references, contact details, the full transcript, and storage paths. Evidence remains separately readable by the owner in Mediverse.
...
Approval schedules deletion by moving verified audio to `pending_deletion` and atomically creating one `audio_deletion` job whose `available_at` matches the configured grace-period deadline. The hourly protected worker leases due jobs with `FOR UPDATE SKIP LOCKED`, removes the exact bucket/path through the **Storage API**, then calls the completion RPC to set `state = 'deleted'`, `deleted_at`, and the job audit fields. An already-missing exact object is an idempotent success. Provider failures retain `pending_deletion`, record only `STORAGE_REMOVE_FAILED`, and retry with a bounded attempt count. The audio metadata row remains for transcript FKs. Deleting `storage.objects` via SQL is never used because it can orphan blob bytes.

The Vercel scheduler and deletion worker require `CRON_SECRET` and `SUPABASE_SECRET_KEY` in the production environment. The secret-key client is server-only and is restricted to the cron route. SQL tests prove queue/state behavior; worker unit tests separately prove the Storage API call occurs before completion. Signed URLs issued before revocation may remain valid until expiry; use short lifetimes. Revocation cannot undo data already sent to a provider.
