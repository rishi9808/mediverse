...
7. Explicit approval calls `approve_note(session_id, latest_revision_id)`. The transaction checks ownership, consent, the exact latest save, complete SOAP, and media readiness. It snapshots the note and schedules verified audio deletion. Retrying the same approval returns the existing snapshot. Further drafts/evidence on that approved session are rejected; amendments are a later workflow.
